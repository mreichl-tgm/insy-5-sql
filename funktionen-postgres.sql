...
-- Kellnerdaten
-- Erstelle zwei Funktionen die in der SELECT-Klausel eingebettet werden, um damit zusätzliche Spalten je Kellner anzeigen zu können. Die aufrufende SELECT-Anweisung soll folgende Ausgabe produzieren: Kellnername, Anzahl der Rechnungen, Status der spätesten Rechnung Erstelle zusätzliche Testdaten (z.B. einen vierten Kellner) bzw. modifiziere die Testdaten so, dass unterschiedliche Statuswerte vorliegen (derzeit sind ja alle Rechnungen vom 7.3.2013).
